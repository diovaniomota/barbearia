-- Pending Migrations for Barbershop App
-- This file contains only the changes that need to be applied to fix migration errors

-- 1. Ensure users table exists with correct schema (IF NOT EXISTS to avoid conflicts)
...
-- 2. Ensure services table has description column (add if missing)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Services table (ensure all columns exist)
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.services ADD COLUMN price numeric(10,2) NOT NULL DEFAULT 0;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.services ADD COLUMN created_at timestamp with time zone DEFAULT now() NOT NULL;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.services ADD COLUMN updated_at timestamp with time zone DEFAULT now() NOT NULL;
    END IF;
END$$;
...
-- 4. Create indexes for better performance (safely)
DO $$
BEGIN
...
-- 6. Apply updated_at triggers to all tables (safely)
DO $$
BEGIN
